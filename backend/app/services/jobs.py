...
class JobService:
    ...
    def _run_demucs(self, input_audio_path: str, job_id: str, target_stems: list[str]) -> tuple[Dict[str, str], str]:
        ...
        if ffmpeg_on_path is None and settings.ffmpeg_binary is not None and settings.ffmpeg_binary.is_file():
            ffmpeg_dir = str(settings.ffmpeg_binary.parent)
            current_path = os.environ.get("PATH", "")
            segments = current_path.split(os.pathsep) if current_path else []
            if ffmpeg_dir not in segments:
                os.environ["PATH"] = f"{ffmpeg_dir}{os.pathsep}{current_path}" if current_path else ffmpeg_dir
            ffmpeg_on_path = shutil.which("ffmpeg")

        if ffmpeg_on_path is None:
            raise RuntimeError(
                "FFmpeg is required for local stem separation and was not found in PATH. "
        ...
        for device in device_candidates:
            try:
                ...
                attempt_errors.append(f"device '{device}': Demucs exited with code {exc.code}")
            except Exception as exc:  # pragma: no cover
                attempt_errors.append(f"device '{device}': {type(exc).__name__}: {exc}")

        details = "; ".join(attempt_errors) if attempt_errors else "no execution attempts were recorded"
        raise RuntimeError(f"Unable to separate stems with Demucs. Attempts: {details}")
