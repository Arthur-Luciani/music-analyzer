...
def _write_index(market_midi_root, entries: list[dict]):
    (market_midi_root / "index.json").write_text(json.dumps(entries), encoding="utf-8")
...
@pytest.mark.asyncio
async def test_returns_not_indexed_when_dataset_missing(isolated_settings):
    use_case = MatchMarketMidiUseCase(_FakeJobService(artist="Any Artist", title="Any Song"))
...
@pytest.mark.asyncio
async def test_returns_no_match_when_query_does_not_match_index(isolated_settings):
    _write_index(isolated_settings.market_midi_root, [{
        "artist": "Some Band",
        "title": "Some Song",
        "artist_norm": "some band",
        "title_norm": "some song",
        "relative_path": "clean_midi/Some Band/Some Song.mid",
    }])
    ...
    use_case = MatchMarketMidiUseCase(_FakeJobService(artist="Totally Unrelated Artist", title="Nothing Alike"))
...
@pytest.mark.asyncio
async def test_returns_candidate_unreadable_when_midi_file_missing(isolated_settings):
    _write_index(isolated_settings.market_midi_root, [{
        "artist": "Test Artist",
        "title": "Test Song",
        "artist_norm": "test artist",
        "title_norm": "test song",
        "relative_path": "clean_midi/Test Artist/Test Song.mid",  # never written to disk
    }])
    _write_drum_analysis(
        isolated_settings.stems_root,
        "session-unreadable",
    ...
    use_case = MatchMarketMidiUseCase(_FakeJobService(artist="Test Artist", title="Test Song"))
...
@pytest.mark.asyncio
async def test_returns_low_confidence_when_duration_incompatible(isolated_settings):
    _write_index(isolated_settings.market_midi_root, [{
        "artist": "Test Artist",
        "title": "Test Song",
        "artist_norm": "test artist",
        "title_norm": "test song",
        "relative_path": "clean_midi/Test Artist/Test Song.mid",
    }])
    _write_drum_analysis(
        isolated_settings.stems_root,
...
@pytest.mark.asyncio
async def test_applies_market_midi_when_match_and_alignment_are_confident(isolated_settings):
